...
export class StellarService {
  ...
  async signAndSubmitServerSide(contractId: string, method: string, args: unknown[]) {
    return this.signAndSubmitAs(this.treasuryKeypair, contractId, method, args);
  }

  /**
   * Generalizes `signAndSubmitServerSide` to sign as any server-held
   * keypair, not just the treasury one — used by the anomaly-oracle
   * scanner (`fraud/oracle-client.service.ts`), which authenticates as
   * its own on-chain identity (granted the oracle role via `set_oracle`,
   * separate from the treasury/admin key) rather than as the admin.
   * Keeping oracle and treasury as two distinct keys means a compromised
   * scanner process can flag/post anomalies (which move no funds — see
   * the contract's "Anomaly oracle" module doc) but can never sign an
   * `issue_voucher`/`burn_expired`/admin call.
   */
  async signAndSubmitAs(keypair: Keypair, contractId: string, method: string, args: unknown[]) {
